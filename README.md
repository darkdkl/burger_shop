...
```
Для расчета расстояния между рестораном и адресом доставки необходим [api ключ геокодера](https://developer.tech.yandex.ru/services/),
Создайте файл .env в каталоге с manage.py и присвойте значения api ключа переменной YANDEX_API_KEY
```
YANDEX_API_KEY ="1s2s3s-dsd3-sqsd3-wce3-cw44cicb"
```
...
```
Создайте таблицу в базе данных для кэша
```
